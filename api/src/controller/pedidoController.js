...
import { buscarItemId } from "../repository/produtoRepository.js";
import { acharCupom, criarNovoPedido } from "../services/novoProdutoService.js";
const server = Router();
...
server.post('/api/pedido/:idUsuario/', async (req, resp) => {
    try {
        ...
        for (let item of info.produtos) {
            const prod = await buscarItemId(item.id);
